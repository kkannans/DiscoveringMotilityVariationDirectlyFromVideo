"""
Fig (flow vectors): Optical flow vector comparison — GT vs model vs OF baseline.

Standalone figure (split from old combined Fig 3).
Requires GPU (model inference). Run via srun on gpu-debug.

OF baseline flow vectors match OpticalFlow.predict_sequence exactly:
...
Outputs -> paths.FIGURES_DIR / fig_flow_vectors.{png,pdf}
"""
...
def select_organoid_and_window():
    ...
    for _, row in candidates.head(20).iterrows():
        ...
        seed = 1  # only seed available in alife repo
...
def get_model_predictions_np(frames_np, ctx_start, seed, device):
    ...
    out_dir = get_output_dir("SimVP_TAU", seed, K, N, residual=True)
...
def main():
    ...
    for ext in ["png", "pdf"]:
        out = OUTPUT_DIR / f"fig_flow_vectors.{ext}"
        fig.savefig(out, dpi=600, bbox_inches="tight", pad_inches=0.03)
    plt.close(fig)
    print(f"Saved: {OUTPUT_DIR / 'fig_flow_vectors.png'}")
    ...
    with open(OUTPUT_DIR / "fig_flow_vectors_metadata.json", "w") as f:
        json.dump(meta, f, indent=2)
    print(f"Metadata: {OUTPUT_DIR / 'fig_flow_vectors_metadata.json'}")
